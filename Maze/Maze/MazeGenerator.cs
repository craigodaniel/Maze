...
namespace Maze.Maze
{
    internal class MazeGenerator
    {
        ...
        public static int[,] GenerateMaze(int height_in_cells, int width_in_cells)
        {
            ...
            while (_stack.Count > 0) // while the stack is not empty
            {
                ...
                if (unvisitedNeighborCells.Count > 0) //if the current cell has any neighbors which have not been visited
                {
                    ...
                    //draw new sections
                    DrawMaze.DrawSection(currentCell);
                    DrawMaze.DrawSection(chosenCell);
                    DrawMaze.DrawSection(Tuple.Create<int, int>((currentCell.Item1 + chosenCell.Item1) / 2, (currentCell.Item2 + chosenCell.Item2) / 2));
                    Thread.Sleep(1);
